...
data flow of \computertext{multiscaleLaplacianFilter} and \computertext{Makefilter} as sketched in t %TODO
\begin{algorithm}
	\caption{Sketch of multiscaleLaplacianFilter and Makefilter functions}\label{alg:dataflow_example}
	\begin{algorithmic}[1]
		\Procedure{Makefilter}{$filter_size$, $degree$, $scale_factor$}
		\EndProcedure
		\Procedure{multiscaleLaplacianFilter}{$Volume$, $degree$, $scales$}
			\ForAll{ scales \puts scale}
			\EndFor
		\EndProcedure
	\end{algorithmic}
\end{algorithm}

% TODO explain how using this view of the call graphs allows for testing from
% the bottom-most level to the top
...
\caption[Callgraph of registration process in \glsentrytext{orionmat}]{
...
\caption[Callgraph of tracing process in \glsentrytext{orionmat}]{
